        ...
        const fetchIdentities = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    if (savedUserId) {
                        const matchedUser = data.find(u => u.id === parseInt(savedUserId));
    ...
    if (!mounted) {
        return (
            <header style={{ 
                ...
                <span style={{ color: "#fff", fontSize: "1.25rem", fontWeight: "bold", fontFamily: "sans-serif" }}>
                    Mini-Amazon
                </span>
            </header>
        );
        ...
        <AppBar position="static" sx={{ backgroundColor: "#131921", padding: "0 10px" }}>
            <Toolbar>
                ...
                <Menu
                    ...
                    slotProps={{
                        paper: {
                            sx: { mt: 1, minWidth: 240, boxShadow: "0px 4px 20px rgba(0,0,0,0.15)" }
                        }
                    }}
                >
                ...
                <Link href="/admin" style={{ textDecoration: "none", color: "inherit", marginRight: "20px" }}>
                    <Box sx={{ 
                        display: "flex", 
                        ...
                        <span style={{ fontSize: "11px", color: "#38bdf8", fontWeight: "bold" }}>System</span>
                        <span style={{ fontSize: "14px", fontWeight: "bold", letterSpacing: "0.5px" }}>Control</span>
                    </Box>
                </Link>

                {/* Returns & Historic Checkout Registry */}
                <Link href="/orders" style={{ textDecoration: "none", color: "inherit", marginRight: "20px" }}>
                    <Box sx={{ display: "flex", flexDirection: "column", padding: "4px 8px", cursor: "pointer", lineHeight: "1.2" }}>
